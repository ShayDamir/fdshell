...
- [x] `copy_file_range` builtin — zero-copy cp between fd vars via `copy_file_range(2)`, no path re-lookup on the hot path (count is an optional positional byte count)
- [ ] `splice` / `sendfile` builtins → zero-copy cat between fd vars, no path re-lookup on the hot path
