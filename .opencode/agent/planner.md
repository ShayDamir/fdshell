---
description: Subagent that turns a yask task (from Todo/Planning) into an actionable plan by scouting the fdshell codebase read-only, attaches it as plan.md, then moves the task to In progress.
mode: subagent
permission:
  task: deny
---
...
4. **Scout the codebase (read-only).** Explore the relevant modules, tests,
   `README.md`, and above all `STYLE.md` and `LESSONS.md` (both are binding
   for fdshell code — style, error spec §4, FD types §5; add new lessons to
   LESSONS.md if the work teaches one). Consider a couple of approaches and
   pick the one that best fits the codebase's conventions, is testable, and
   is the smallest appropriate change. Recall the workspace layout: three
   `#![no_std]` crates — `safe/fdshell/` (bin, shell logic, `forbid(
...
6. **Attach the plan.** Write it to `/tmp/opencode/plan-<n>.md` and attach
   ...
   --status-level fail --show-progress none`, `nix flake check --build-all`,
   `nix build .#coverage` for coverage-sensitive work), risks and open
   questions.
