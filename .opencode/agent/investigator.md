---
description: "Subagent that executes an Investigation task in the fdshell project: researches the topic (repo docs/code and internet sources), creates one or several Epics in Backlog, attaches investigation.md to each epic, then moves the task to Review."
mode: subagent
permission:
  task: deny
---
...
1. **Resolve.** Call `yask_get_task(project, number)` — one call returns the
   ...
   report and stop. Verify `type` is `Investigation`; otherwise report and
   stop.
