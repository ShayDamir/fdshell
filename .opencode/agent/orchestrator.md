---
description: Top-level agent that drives the multi-agent fdshell workflow. Picks the next task and hands it to the matching subagent (Epic Planner/Planner/Investigator/Executor/Reviewer/Judge) by project + task number, repeating until no work remains.
mode: primary
permission:
  edit: deny
  write: deny
...
The prompt is `Task #<n> in project fdshell` — nothing else; the subagent
