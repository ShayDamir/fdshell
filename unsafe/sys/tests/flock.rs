...
fn exclusive_lock_succeeds_and_releases() {
    ...
    sys::fileops::flock(&fd, LOCK_EX).unwrap();
    sys::fileops::flock(&fd, LOCK_UN).unwrap();
}
...
fn second_exclusive_nowait_is_ewouldblock() {
    ...
    sys::fileops::flock(&fd1, LOCK_EX).unwrap();
    let fd2 = open_lock(&path);
    let err = sys::fileops::flock(&fd2, LOCK_EX + LOCK_NB).unwrap_err();
...
fn shared_locks_do_not_conflict() {
    let path = lock_file();
    let fd1 = open_lock(&path);
    sys::fileops::flock(&fd1, LOCK_SH).unwrap();
    let fd2 = open_lock(&path);
    sys::fileops::flock(&fd2, LOCK_SH + LOCK_NB).unwrap();
}
...
fn unlock_allows_exclusive_nowait() {
    ...
    let fd1 = open_lock(&path);
    sys::fileops::flock(&fd1, LOCK_EX).unwrap();
    sys::fileops::flock(&fd1, LOCK_UN).unwrap();
    let fd2 = open_lock(&path);
    sys::fileops::flock(&fd2, LOCK_EX + LOCK_NB).unwrap();
}
