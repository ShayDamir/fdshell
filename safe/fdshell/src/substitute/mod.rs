...
pub(crate) fn substitute_arg(
    ...
    let bytes = arg.as_bytes().map_err(|_| ResolveError::RefNotFound)?;
    let mut out = Vec::new();
    ...
    let mut state = cell.borrow().map_err(|_| ResolveError::RefNotFound)?;
    while let Some(b) = peek.next() {
        match b {
            ...
            b'$' if peek.peek() == Some(&b'(') => {
                ...
                state = cell.borrow().map_err(|_| ResolveError::RefNotFound)?;
            }
            b'$' => dollar::dollar_subst(&mut peek, &state, &mut out)?,
            _ => out.push(b),
        }
    }
    CString::new(out).map_err(|_| ResolveError::NulByte)
}

pub fn substitute_args(
    args: &[ShortCStr],
    cell: &ForkCell<ShellState>,
) -> Result<Vec<CString>, ResolveError> {
