...
pub(crate) fn run_one(line: &[u8], state: &mut ShellState) -> Result<(), i32> {
    match crate::parse::parse(line)? {
        ...
        crate::parse::ParsedLine::While(whileblock) => {
            crate::loop_::run_loop(&whileblock.condition, &whileblock.body, true, state)?;
        }
        crate::parse::ParsedLine::Until(untilblock) => {
            crate::loop_::run_loop(&untilblock.condition, &untilblock.body, false, state)?;
        }
        crate::parse::ParsedLine::If(ifblock) => {
            crate::if_exec::run_if(&ifblock, state)?;
        }
    }
    Ok(())
}
