...
6.5 `ShortCStr` supports mutations: `push(byte)` and `push_slice(&[u8])` check for NUL bytes and can fail; `push_str(&ShortCStr)`, `push_cstr(&CStr)`, and `concat(&[&ShortCStr])` are infallible because their sources are guaranteed NUL-free by type invariant.
