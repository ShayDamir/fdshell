...
## cargo-mutants can't run in the dev container: memfd-dependent sys tests fail the baseline
cargo-mutants runs the unmutated suite first and aborts if any test fails (`cargo test failed in an unmutated tree, so no mutants were tested`). `memfd_create` is blocked in this dev container (errno 14), so every pre-existing sys/fdshell test that opens its backing object with `memfd_create` — `fallocate_handler_succeeds_on_memfd`, `flock_handler_locks_and_unlocks_memfd`, `fsync_flushes_memfd`, … — fails, and `cargo mutants` on e.g. `fileops.rs` or `child/copy_file_range.rs` never reaches the mutants. That is environmental, not a defect: the copy_file_range sys wrapper has its own dedicated integration tests that use regular files (see `Dir` in `unsafe/sys/tests/copy_file_range.rs`, which deliberately avoids memfd) and pass; the exec logic lives in `builtins/…/mod.rs`, whose baseline is memfd-free and is fully mutant-tested. To mutation-test the sys crate here, run on a host with `memfd_create`, or scope the run to a file whose own tests don't need it.
