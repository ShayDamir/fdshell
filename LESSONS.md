...
When wrapping one error type in another with `.change_context()`, avoid variant pairs where both have the same Display message. The `error_stack` `{:?}` output shows both levels, so if both `ChildProcessError::NotFound` and an inner source say `"command not found"`, the user sees it twice. The outer variant should describe *what* failed (e.g., `"failed to resolve command path"`), while the inner source describes *why* (e.g., `"not found"`). This mirrors the natural language pattern: the chain reads as "X failed → because Y" rather than "Y → because Y". `ChildProcessError` has both `ResolveFailed` (outer, what) and `NotFound` (inner, why) to make this pattern explicit per-variant.
